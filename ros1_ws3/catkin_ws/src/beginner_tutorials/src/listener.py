...
def image_callback(img):
    ...
    cv2.waitKey(1)
    rospy.loginfo("got the topic /image_raw ...")
    object_detect(image)
...
def object_detect(image):
        ...
        steering_pub=rospy.Publisher('twist_topic',Twist,queue_size=10)
        twist_msg=Twist()
        twist_msg.angular.z=steering_angle
        steering_pub.publish(twist_msg)
        head_image=display_heading_line(lane_lines_image,steering_angle)
        #steer(image,lane_lines)
        # result.write(head_image)
...
def listener():
    ...
    rospy.Subscriber('image_topic', Image, image_callback,queue_size=None)
    steering_pub=rospy.Publisher('twist_topic',Twist,queue_size=10)
